...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(f'logs/chat_debug_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log', encoding='utf-8'),
        logging.StreamHandler(sys.stdout)
    ]
)

# Log git commit hash or file version if available
try:
    with open(__file__, 'r') as f:
        file_content = f.read()
    file_hash = hash(file_content)
    logging.info("Script version hash: %s", file_hash)
except Exception as e:
    logging.warning("Could not log script version: %s", str(e))
...
def get_available_personas() -> Dict[str, Path]:
    ...
    if not personas_dir.exists():
        logging.error(f"Personas directory not found: {personas_dir}")
        personas_dir.mkdir(exist_ok=True)
        logging.info(f"Created personas directory: {personas_dir}")
    
    personas = {
        "Jane": personas_dir / "flirty_persona.json",
        "Professor Riley (Mentor)": personas_dir / "mentor_persona.json",
        "Emma (Gamer Friend)": personas_dir / "friend_persona.json",
        "Dr. Amelia (Professional)": personas_dir / "professional_persona.json",
        "Yuki (Anime Fan)": personas_dir / "anime_persona.json"
    }
    
    # Filter out any personas whose files don't exist
    available_personas = {k: v for k, v in personas.items() if v.exists()}
    
    if not available_personas:
        logging.warning("No persona files found in directory: %s", personas_dir)
    else:
        logging.info("Found %d persona(s): %s", len(available_personas), ", ".join(available_personas.keys()))
    
    return available_personas

def select_persona() -> Path:
    """Let user select a persona from available options"""
    ...
    print("-" * 50)
    for idx, (name, _) in enumerate(personas.items(), 1):
        print(f"{idx}. {name}")
    print("-" * 50)
    
    while True:
        try:
            choice = int(input(f"\nSelect a persona (1-{len(personas)}): "))
            if 1 <= choice <= len(personas):
                selected = list(personas.values())[choice - 1]
                selected_name = list(personas.keys())[choice - 1]
                print(f"\nSelected persona: {selected_name}\n")
                logging.info("Selected persona: %s (%s)", selected_name, selected)
                return selected
            else:
                print(f"Invalid choice. Please select a number between 1 and {len(personas)}.")
        except ValueError:
...
def load_system_message(reload: bool = False) -> str:
    ...
    try:
        ...
        if not hasattr(load_system_message, "persona_path") or reload:
            load_system_message.persona_path = select_persona()
            
        if not load_system_message.persona_path.exists():
            raise FileNotFoundError(f"Persona file not found: {load_system_message.persona_path}")
            
        with open(load_system_message.persona_path, "r", encoding="utf-8") as f:
...
class PersistentChat:
    def __init__(self, model: str = XAI_MODEL, memory_file: str = "chat_memory.json"):
        ...
        # Use OpenAI client with X.AI base URL - matching working copy exactly
        self.client = OpenAI(
            api_key=XAI_API_KEY,
    ...
    def _make_api_request(self, messages: List[Dict]) -> str:
        ...
        for attempt in range(RETRY_ATTEMPTS):
            try:
                ...
                # Make request using OpenAI client - matching working copy
